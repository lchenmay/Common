...
try {
    ...
    $looksLikeSourceWrite = $isFileEdit -or ($toolText -match '(?i)\b(Set-Content|Add-Content|Out-File|Copy-Item|Move-Item|Remove-Item|New-Item|Rename-Item|git\s+apply)\b')
    if ($looksLikeSourceWrite) {
        foreach ($root in @($policy.siblingRepositoryRoots)) {
            $sibling = ConvertTo-PolicyNormalizedText ([string] $root)
            $leaf = Split-Path -Leaf ([string] $root)
            $relative = "../$($leaf.ToLowerInvariant())/"
            if ($normalized.Contains($sibling) -or $normalized.Contains($relative)) {
                $findings.Add([pscustomobject]@{ rule = 'cross-repository-write'; severity = 'deny'; detail = "Write targets sibling repository $root. Open a separate task rooted there." })
            }
        }
    }
