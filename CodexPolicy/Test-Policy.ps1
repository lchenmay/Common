...
try {
    ...
    $siblingResult = Invoke-Hook 'PreToolUse.ps1' $sibling | ConvertFrom-Json
    Assert-True -Condition ($siblingResult.hookSpecificOutput.permissionDecision -eq 'deny') -Name 'cross-repository patch is denied'
    ...
    Write-Output 'Policy tests passed: 8'
}
finally {
