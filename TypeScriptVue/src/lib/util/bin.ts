...
export const bin__option = (bin__item: Function) => (bi: BinIndexed) => {
    if(bin__bool(bi))
        return bin__item(bi)
    else
        return null
...
// This is a 2-order function with 2 arrows in the signature
export const bin__array = (bin__item: Function) => (bi: BinIndexed) => {
    let res = []
...
export const bin__List = (bin__item: Function) => (bi: BinIndexed) => {
    let res = []
    if (bi.bin.byteLength >= 4) {
        let length = bin__int32(bi)
        for (let i = 0; i < length; i++) {
