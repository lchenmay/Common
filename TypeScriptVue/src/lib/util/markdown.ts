...
export const markdown__html = (str: string) => {
  ...
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    if(state == "" && line.startsWith("$$")){
      ...
      let alt = s.replace(/"/g," ").replace(/'/g," ").replace(/</g," ").replace(/>/g," ")
      let img = 
    ...
    if (latexMatches) {
      for (const s of latexMatches) {
        let tex = encodeURI(s.substring(1,s.length - 1))
        let alt = s.substring(1,s.length - 1).replace(/"/g," ").replace(/'/g," ").replace(/</g," ").replace(/>/g," ")
        let img = 
          "<img class='img-inline' src='http://latex.codecogs.com/gif.latex?" + tex +"' alt='" + alt + "'>"
