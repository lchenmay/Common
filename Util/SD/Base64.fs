  ...
  let charseturl = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_" |> Seq.toList
  ...
  let converturl data =
    data
    |> Seq.choose (fun e -> Map.tryFind e tableurl)
    |> String.Concat
  ...
  let decode64string (data:string) =
      data
      |> convert
      |> getQuotients
      |> getBuffers
  ...
  let decode64urlbytes (data:string) =
    data
    |> converturl
    |> getQuotients
    |> getBuffers
  ...
  let decode64urlstring (data:string) =
    data
    |> converturl
    |> getQuotients
    |> getBuffers
  ...
  let str__base64Stro (data:string) =
    try
      data 
  ...
  let base64Str__stro (base64Str:string) =
    try
      base64Str
  ...
  let isBase64Str (input: string) =
    try
        Convert.FromBase64String input |> ignore
        true
    with
    | _ -> false
