...
let tryFindBoolByAtt attName json =
    match json with
    | Json.Braket items -> 
        match 
            items 
...
let tryFindBoolByAttWithDefault dft attName json = 
    match tryFindBoolByAtt attName json with
    | "true" -> true
    | "false" -> false
    | _ -> dft
...
let tryFindTrueByAtt attName json =
    match json with
    | Json.Braket items -> 
        match 
            items 
...
let tryFindFalseByAtt attName json =
    match json with
    | Json.Braket items -> 
        match 
            items 
...
let json__ModDictInt64o json__valo dict json = 
    ...
    | Some dict -> dict |> dict__ModDictInt64 |> Some
    | None -> None
...
let json__ModDictStro json__valo dict json = 
    ...
    | Some dict -> dict |> dict__ModDictStr |> Some
    | None -> None
