...
open Util.Orm
open Util.TypeSys
open Util.CodeRobotI
open Util.CodeRobotIIFs
open Util.CodeRobotIITs
open Util.CodeRobotIII
...
let save srcs = 
    ...
    |> Array.iter(fun src ->
        src
        |> src__txt crlf
        |> FileSys.try_write_text (src.filename)
        |> ignore)
