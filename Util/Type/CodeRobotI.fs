...
let items__fieldo items =
    ...
    if fieldname.Length * e.Length > 0 then
        ...
        | "Caption" -> (fieldname, FieldDef.Caption(checkfield items "length" |> Text.parse_int32),items) |> Some
        | "Chars" -> (fieldname, FieldDef.Chars(checkfield items "length" |> Text.parse_int32),items) |> Some
        | "Link" -> (fieldname, FieldDef.Link(checkfield items "length" |> Text.parse_int32),items) |> Some
