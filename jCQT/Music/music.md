...
- **算法（判据链 v6：宽松峰值候选 + 子带双峰值 + 连续性/贯穿触达/两端细度 + 列宽严格性 + 结束线兜底）**：
  - **职责划分（铁律）**：**候选**用「垂直累计分布的（宽松）峰值」——小节线是不连音符的孤立细竖线，列投影呈尖锐单峰、至少一侧陡峭下落；**排除**用「贯穿性」（判据2+判据3，必须跨高低声部贯通）+「列宽严格性」（判据5）。宽音符簇是平台/多峰、任一侧都不陡，仍不进候选。候选尽量放宽、排除保持严格，避免漏检。
  - **精确纵向跨度（铁律）**：小节线必须**精确连接「高音最上线」`min(lineYs)` 与「低音最下线」`max(lineYs)`**（即 10 根谱线的高音最顶线与低音最底线），不再使用含 padding / 高低音谱表外（歌词、力度记号、加线）的整段 region 带。列投影 `colInk`/`upColInk`/`downColInk` 仅在 `[yT, yB)` 内累加，谱表外噪声不再污染投影。无 `lineYs` 时回退到原 region 带兜底。
  - 判据1（候选，尽量放宽）：列暗像素投影 `colInk` 满足——(a) 幅值 `colInk[x] ≥ 0.15×bandH`（仅排除孤立单音符头，其中 `bandH = yB−yT` = 高音最上线↔低音最下线 高度）；(b) 弱局部峰 `colInk[x] ≥ 左邻 且 ≥ 右邻`（允许平台，覆盖宽/双像素小节线两端）；(c) **任一侧略陡**：在 `±peakWin(=4px)` 窗口内，峰值相对某一侧最小值的落差 `≥ 0.15×bandH`。条件(c) 仅要求"有点峰形"（小节线 1~2px 宽、任一侧几像素内投影骤降 → 过；宽音符簇在 ±4px 内维持高投影 → 仍不进候选），不再要求两侧皆陡，避免紧邻音符簇的小节线被误杀。**浅蓝标记**：所有候选列先以浅蓝竖线绘制，最终经判据2/3/5 收紧后再以深蓝绘制，便于直观对比、校准候选阈值。
  - 判据2：把 band 在**高低声部空隙中心**一分为二（上支/下支）——大谱表取第5线/第6线之间 `midY=(lineYs[4]+lineYs[5])/2`，单行谱取中间线之间 `midY=(lineYs[2]+lineYs[3])/2`，退化情形取整段中点；要求 `上支colInk ≥ 0.3×上支高 且 下支colInk ≥ 0.3×下支高`。小节线上下两支都有强墨；音符符干/和弦只占单支谱表高度，被滤掉。
  - 判据3（加强版：连续性 + 贯穿触达 + 两端细度——专治长符干与跨谱表和弦）：对候选列用 `longestVerticalRun` 取最长连续暗段 `(runLen, rTop, rBot)`，须同时满足：
    - **判据3a 连续性**：`runLen ≥ 0.65×bandH`（v6 由 `0.4` 收紧到 `0.8`、再实测放松到 `0.65`：小节线精确贯通整段 10 线高度，但常被音符头遮挡 / 扫描抗锯齿在顶底断开，最长连续段略低于 `bandH`，故取 `0.65` 容忍局部断点；仍显著高于单谱表符干 ≈ `0.4×bandH`）。真实小节线是 1px 宽连续竖线、`maxRun` 接近 `bandH`；单谱表符干只跨 1~2 个八度、`maxRun` ≪ `bandH` 被拒。
    - **判据3b 贯穿触达**：连续段必须"触达"上下两端——`rTop ≤ yT+0.20×bandH` 且 `rBot ≥ yB−0.20×bandH`。小节线真正从「高音最上线」连到「低音最下线」；长符干只在一侧（如仅上支）、不触达对端 → 拒。
    - **判据3c 两端细度**：连续段顶端 / 底端各采样 3 行的横向墨宽必须 ≤ `maxEndWidth(=3px)`（行内以候选列为中心向左右扩展到阈值以下测 `rowWidthAt`）。小节线全程 1~2px 细 → 过；**跨双谱表和弦**的符干虽细、且 `maxRun` 可接近 `bandH`（曾能骗过判据3a+判据2），但其顶端 / 底端是宽 `notehead` 团（≥4px）→ 判据3c 拒。单符干顶端 `notehead` 同理被拒。**谱线穿越处理（关键）**：采样行可能恰落在贯穿全宽的五线谱线上（小节线顶端就在高音最上线），此时 `rowWidthAt` 会被谱线拉伸到几十~上百 px —— 这种"极宽(>20px)"是谱线穿越而非 `notehead`，须**跳过（不计入失败）**；只有"中等宽(>3px 且 ≤20px)"才判为 `notehead` 失败。否则真实小节线会被谱线误杀。
    - 判据3a+3b 已根治长符干；3a+3c 根治跨谱表和弦；密集 16/32 分音符串由判据5 列宽兜底拦截——三重保险。
  ...
  - **startX 铁律**：`startX = min(w−5, max(braceZone, hr+8))`。上限钳制 `w−5` 是 v6 修复——此前 `min w` 在 `hr+8≥w` 时令 `startX=w`、候选循环 `for x in w..w−1` 空转，整段无声漏检（无任何蓝线）。`hr`（头部块右界，来自 `detectClefRightByRegion` 红框右缘，单一事实来源，稳健）异常偏大时会触发钳制告警，提示该段可能漏检左部小节线。 braceZone = `max 20 (0.04×w)` 为左缘兜底。
  - **诊断日志**：每段输出 `[小节线] 段#ri bandH/上支高/下支高 startX/hr/lyN/候选/判据2过/判据3过/判据5过/兜底/绘制`（`startX`=候选起始列、`hr`=头部块右界、`lyN`=lineYs 数；判据2过=上下支双峰都达 0.3、判据3过=过 3a+3b+3c、判据5过=列宽≤2）。整段跨度退化时输出 `[小节线] 段#ri 跳过：纵向跨度退化 ...`；`hr+8≥w−5` 钳制时输出 `[小节线] 段#ri 警告：头部块右界过大 ...`。此二者是定位"整片无蓝线"段的关键：前者说明 lineYs 缺失/region 退化，后者说明头部块右界估计异常。
- **调试日志**：每段输出 `[小节线] 段#ri bandH/上支高/下支高 候选/判据2过(剔N)/判据3过(剔N)/判据5过(剔N)/结束线兜底/绘制 数量`（判据2过=上下支双峰都达 0.3、判据3过=过 3a+3b+3c、判据5过=列宽≤2，"结束线兜底"=判据6 追加的列数）；全部段完成后输出 `[小节线] 完成：N 段共绘制 M 根蓝色竖线`。日志写入主程序日志区（Avalonia 日志组件），**不使用 printfn**。
